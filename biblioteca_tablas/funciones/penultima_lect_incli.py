"""Función de celda: fecha de la penúltima lectura activa de un inclinómetro JSON.

Devuelve la fecha ISO de la segunda campaña más reciente cuya clave sea ≤ fecha_fin
y tenga campaign_info.active=true. Devuelve None si no existen al menos 2 campañas.
"""
...
CELL_FUNCTION_METADATA: dict[str, Any] = {
    "nombre": "penultima_lect_incli",
    "devuelve": "texto",
    "descripcion": (
        "Fecha (ISO) de la penúltima campaña activa del inclinómetro con fecha "
        "igual o anterior a fecha_fin. Devuelve None si hay menos de 2 campañas."
    ),
    "parametros": [
        {
            "nombre": "sensor",
...
def _cargar_campanas_activas(sensor: str, fecha_fin: str) -> list[str]:
    ...
    if not ruta.is_file():
        logger.warning("[penultima_lect_incli] Archivo no encontrado: %s", ruta)
        return []
    ...
    try:
        ...
        logger.warning("[penultima_lect_incli] Error leyendo %s: %s", ruta, exc)
        return []
...
def evaluate(
    ...
    """Devuelve la fecha ISO de la penúltima campaña activa ≤ fecha_fin.
    ...
    Returns:
        Fecha ISO (str) de la penúltima campaña activa, o ``None`` si hay <2.
    """
    ...
    return campanas[-2] if len(campanas) >= 2 else None
