...
from utils.funciones_comunes import get_color_for_index, buscar_referencia, calcular_incrementos, df_to_excel
from utils.funciones_correcciones import grafico_violines, dict_a_df, creacion_df_bias, calculos_bias
...
def layout():
    return dmc.MantineProvider(
        children=dmc.Grid([
            ...
            dcc.Store(id="json_final", storage_type='memory'),# Campaña seleccionada con las correcciones spk+bias
            ...
            dmc.Grid([
                ...
                dmc.Col([
                    ...
                    dmc.Group(
                        children=[
                            dmc.Text("Ver cambios", style={"fontWeight": "bold"}),
                            dmc.Button("Run Bias", id="run_bias", c="blue", variant="outline")
                        ],
                        justify="flex-start",
                        gap="1",
                        style={"marginTop": "10px"}
...
def register_callbacks(app):
    ...
    def actualizar_camp_corregida(data, cellValueChanged, camp_graficar, table_data):#, camp_corregida):
        ...
        for entrada in correccion[camp_graficar]["calc"]:
            ...
            entrada["checksum_a"] = entrada["a0"] + entrada["a180"],
            entrada["checksum_b"] = entrada["b0"] + entrada["b180"]
        # busco referencia
    ...
    """
    @app.callback(
        [Output('bias-table', 'columnDefs'),
        Output('bias-table', 'rowData')],
        [Input("corregir-tubo", 'data'),
         Input('camp_a_graficar', 'value')])
    def update_bias_table(camp_corregida, camp_selecc):
        # la tabla se carga al subir un archivo o cambiar la fecha seleccionada
        if not camp_corregida:
            return dash.no_update, dash.no_update

        # Obtener los valores de 'depth' del diccionario camp_corregida
        data = camp_corregida[camp_selecc]['calc'] # datos de la campaña seleccionada

        # Extraer valores únicos y ordenados
        depth_values = sorted({item['depth'] for item in data})

        # Calcular los valores predeterminados
        prof_inf_default = max(depth_values)
        prof_sup_default = min(depth_values)

        # Definir las columnas con los dropdowns
        columnDefs = [
            {'headerName': 'Corrección', 'field': 'Correccion', 'editable': False},
            {
                'headerName': 'Selec.',
            ...
            },
            {
                ...
                'cellEditorParams': {'values': depth_values},
                'valueSetter': f'function(params) {{ return {prof_inf_default}; }}'
            },
            {
                'headerName': 'Prof. sup.',
                'field': 'Prof_sup',
                ...
                'cellEditor': 'agSelectCellEditor',
                'cellEditorParams': {'values': depth_values},
                'valueSetter': f'function(params) {{ return {prof_sup_default}; }}'
            },
            {'headerName': 'Delta', 'field': 'Delta', 'editable': True}
        ]

        # Actualizar las filas con valores predeterminados
        rowData = [
            {'Correccion': 'Bias_1_A', 'Selec': False, 'Prof_inf': prof_inf_default, 'Prof_sup': prof_sup_default, 'Delta': ''},
            {'Correccion': 'Bias_1_B', 'Selec': False, 'Prof_inf': prof_inf_default, 'Prof_sup': prof_sup_default, 'Delta': ''},
            {'Correccion': 'Bias_2_A', 'Selec': False, 'Prof_inf': prof_inf_default, 'Prof_sup': prof_sup_default, 'Delta': ''},
            {'Correccion': 'Bias_2_B', 'Selec': False, 'Prof_inf': prof_inf_default, 'Prof_sup': prof_sup_default, 'Delta': ''}
        ]
    ...
    @app.callback(
        ...
        [Input("json_spikes", 'data'),
         Input('run_bias', 'n_clicks'),
         Input('bias-table', 'cellValueChanged')], # si no hay cambios, coge la anterior
        [State('bias-table', 'rowData'),
         State('corregir-tubo', 'data'),
    ...
    def graficos_bias(camp_corregida, run_bias, bias_table, cellValueChanged, corregir_tubo, escala_desplazamiento,
                      valor_positivo_desplazamiento, valor_negativo_desplazamiento):
        ...
        # 1. Extraer la fecha seleccionada (primera clave del diccionario camp_corregida)
        fecha_selec = list(camp_corregida.keys())[0]

        # 2. Buscar la fecha de referencia usando buscar_referencia
        fecha_referencia = buscar_referencia(corregir_tubo, fecha_selec)

        # 3. Crear DataFrame df_bias con valores de referencia y corrección
        calc_ref = corregir_tubo[fecha_referencia]['calc'] # datos de la campaña de referencia
        calc_corr = camp_corregida[fecha_selec]['calc'] # datos de la campaña a corregir
        ...
        # 5. Corrección FALTA METER LA PERSONALIZADA EN TABLA-BIAS (CON RUN BIAS)
        # 5.a. Devuelve el valor de la corrección sugerida
        # Calculo el valor de incr en profundidad h-5 m. Criterio arbitrario, considero los últimos 5 metros como anclados
        ...
        #print("El último valor de la columna 'depth' es:", h-5)
        delta_a = df_bias.loc[df_bias['depth'] == h-5, 'avg_Incr_A'].iloc[0]
        delta_b = df_bias.loc[df_bias['depth'] == h - 5, 'avg_Incr_B'].iloc[0]
        #print("El valor de delta es:", delta_a)
        # serie recta
        last_index = df_bias.index[-1]  # Última fila del índice
        df_bias['recta_a'] = delta_a * (last_index - df_bias.index) # es una recta, sólo depende de la pendiente (delta)
        df_bias['recta_b'] = delta_b * (last_index - df_bias.index)
        ...
        if not cellValueChanged:
            # el dcc temporal guarda los valores de delta_a, delta_b
            calculated_bias = [delta_a, delta_b]
        else:
            calculated_bias = dash.no_update

        #print("Deltas actualizados:", calculated_bias)
        ...
        return fig_a,fig_a_estad, fig_b, fig_b_estad, calculated_bias
    ...
    def emergente_bias(n_clicks, selected_vars, data_json, data_corr, lista_fechas, fecha_selec):
        ...
        #print("variable seleccionada", selected_vars)
    ...
    def cambios_json_bias(corr_spikes , cellValueChanged,  empotramiento, bias_table, corregir_tubo):
        ...
        if corr_spikes:
            # CASO I. Ya hay una campaña cargada
            ...
            if (df_bias_table['Prof_inf'].sum() + df_bias_table['Prof_sup'].sum()) == 0:
                ...
                df_bias_table.loc[0, 'Prof_inf'] = df_bias['depth'].max()
                df_bias_table.loc[0, 'Prof_sup'] = df_bias['depth'].min()
                df_bias_table.loc[0, 'Delta'] = round(df_bias.loc[df_bias['depth'].idxmin(), 'recta_a'],2)
                # Fila Bias_1_B
                df_bias_table.loc[1, 'Selec'] = True
                df_bias_table.loc[1, 'Prof_inf'] = df_bias['depth'].max()
                df_bias_table.loc[1, 'Prof_sup'] = df_bias['depth'].min()
                ...
                #print ('--------------------------')
                #print({fecha_selec:{'calc':dict_df_bias, 'bias':rowData_actualizado}})
            ...
            return resultado, dash.no_update
        else:
            return dash.no_update, dash.no_update
