...
#from TD_medias.vol_td.main import colores_leyenda
    ...
    def update_hovercard_and_store(contents, filename):
        if contents and filename:
            ...
            try:
                ...
                nuevo_diccionario = {"info": data["info"],}
                ...
                for index, (clave, valor) in enumerate(data.items()):
                    if clave != "info" and "calc" in valor and valor.get("campaign_info", {}).get("active") == True:
                        ...
                        nuevo_diccionario[clave] = {
                            ...
                            #"color": get_color_for_index_(index)
                        }
    ...
    def actualizar_graficos(fechas_seleccionadas, fechas_colores, slider_value, data, alto_graficos, color_scheme,
        ...
        for fecha in fechas_seleccionadas:
            if fecha in data and "calc" in data[fecha]:
                ...
                fig1_a.add_trace(
                    go.Scatter(x=desp_a_list, y=cota_abs_list, mode="markers+lines", name=f"{fecha} - Desp A",
                               line=dict(c=color, width=grosor)))
                fig1_b.add_trace(
                    go.Scatter(x=desp_b_list, y=cota_abs_list, mode="markers+lines", name=f"{fecha} - Desp B",
                               line=dict(c=color, width=grosor)))

                # Gráfico 2: Incrementales
                fig2_a.add_trace(
                    go.Scatter(x=incr_dev_a_list, y=cota_abs_list, mode="markers+lines", name=f"{fecha} - Incr Dev A",
                               line=dict(c=color, width=grosor)))
                fig2_b.add_trace(
                    go.Scatter(x=incr_dev_b_list, y=cota_abs_list, mode="markers+lines", name=f"{fecha} - Incr Dev B",
                               line=dict(c=color, width=grosor)))
                ...
                fig3_total.add_trace(
                    go.Scatter(x=desp_total_list, y=cota_abs_list, mode="markers+lines", name=f"{fecha} - Desp Total",
                               line=dict(c=color, width=grosor)))
