...
def layout():
    return dmc.MantineProvider(
        ...
        children=dmc.Container([
            ...
            dmc.Paper(p="md", withBorder=True, shadow="sm", radius="md", mb=20, children=[
                dmc.Text("Paso 3: Configurar tipo de flanco y renombrar cada deformada", fw=500, mb=10),
                dmc.LoadingOverlay(
    ...
    def load_excel(contents, filename):
        ...
        try:
            ...
            for deformada in deformadas:
                selectors.append(
                    dmc.Paper(p="xs", withBorder=True, shadow="xs", radius="sm", mb=10, children=[
                        dmc.Group([
                            dmc.Text(f"Deformada original: {deformada}", fw=500, style={"width": "200px"}),
                            dmc.TextInput(
                                id={"type": "deformada-name-input", "index": deformada},
                                label="Nuevo nombre:", value=deformada, style={"width": "300px"}
                            ),
                            dmc.Select(
                                ...
                                value="flanco_positivo", style={"width": "300px"}
                            )
                        ])
                    ])
                )
            return (f"Excel procesado: {filename}. Se encontraron {len(deformadas)} deformadas.", False, selectors, False, deformadas)
        except Exception as e:
            return (f"Error: {e}", False, [], True, [])

    @callback(
        [Output('i-flancos-status', 'children'), Output('i-flancos-status', 'hide'),
         Output('i-umbrales-output', 'children'), Output('i-save-json', 'disabled'), Output('i-json-data-umbrales', 'data')],
        ...
        State('i-stored-json-data', 'data'), State('i-deformadas-list', 'data'), State('i-upload-excel', 'contents'),
        prevent_initial_call=True
    )
    def process_flancos(n_clicks, name_values, flanco_values, datos_json, deformadas, excel_contents):
        ...
        try:
            ...
            deformadas_dict = {name_map[deformadas[i]]: flanco_values[i] for i in range(len(deformadas))}
            registros_renombrados = [
                {name_map.get(k, k): v for k, v in rec.items()} for rec in registros_redondeados
            ]
            umbrales = {"deformadas": deformadas_dict, "valores": registros_renombrados}
            datos_json_mod = datos_json.copy()
            datos_json_mod['umbrales'] = umbrales
            return ("Configuración de flancos confirmada", False, json.dumps(umbrales, indent=2), False, datos_json_mod)
        except Exception as e:
            return (f"Error: {e}", False, "{}", True, {})
