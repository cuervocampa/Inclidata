...
def register_callbacks(app):
        ...
        if triggered and 'import-third-button' in triggered[0]['prop_id'] and (
            ...
            try:
                ...
                for fecha in fechas_agg:
                    ...
                    # Referencia a aplicar al cálculo
                    if checkbox_ref_value == 1 or not fecha_referencia:
                        ...
                        # Añadir la información al diccionario de la campaña
                ...
                # Generar gráficos
                graphs = importar_graficos(tubo, fechas_agg) # utils/funciones_graficos.py

                # Crear tabla de configuración de campañas
                campaign_config_rows = []
                for i, fecha in enumerate(fechas_agg):
                    campaign_config_rows.append(
                        Grid([
                            Col(span=2, children=[
                                Text(fecha, size="sm", fw=500)
                            ]),
                            Col(span=2, children=[
                                dmc.TextInput(
                                    ...
                                    size="sm",
                                    style={"width": "100%"},
                                    type="text",  # Usar type en lugar de inputMode
                                )
                            ]),
                            Col(span=2, children=[
                                dmc.TimeInput(
                                    id={'type': 'time-input', 'index': i},
                                    value=datetime.strptime(fecha.split('T')[1], "%H:%M:%S").time(), # Convierte string a time
                                    size="sm",
                                    style={"width": "100%"}
                                )
                            ]),
                            ...
                            Col(span=2, children=[
                                Select(
                                    ...
                                    data=[
                                        {"label": "En cuarentena", "value": True},
                ...
                return Paper(
                    ...
                    children=[
                        ...
                        # Tabla de configuración de campañas
                        Card(
                            withBorder=True,
                            shadow="sm",
                            radius="md",
                            ...
                            children=[
                                ...
                                CardSection(
                                    ...
                                    children=[
                                        ...
                                        Grid([
                                            Col(span=2, children=[
                                                Text("Fecha Original", fw=700, size="sm")
                                            ]),
                                            Col(span=2, children=[
                                                Text("Fecha TunnelData", fw=700, size="sm")
                                            ]),
                                            Col(span=2, children=[
                                                Text("Hora TunnelData", fw=700, size="sm")
                                            ]),
                                            Col(span=2, children=[
                                                Text("Activa", fw=700, size="sm")
                                            ]),
                                            Col(span=2, children=[
                                                Text("Cuarentena", fw=700, size="sm")
                                            ]),
                                            Col(span=2, children=[
                                                Text("Subir Campaña", fw=700, size="sm")
                                            ])
                                        ], gutter="xs", mb=10, ta="center"),
                ...
                return Alert(
                    ...
                    icon=DashIconify(icon="mdi:alert-circle"),
                    children=[
                        Text(f"Se produjo un error: {str(e)}")
                    ]
                ), None
    ...
    def update_campaign_settings(n_clicks, dates, times, active_values, quarentine_values, upload_values,
        ...
        # Validar que n_clicks no sea None
        if n_clicks is None:
            raise dash.exceptions.PreventUpdate  # Evita que el callback se ejecute
        ...
        if n_clicks is not None and n_clicks > 0 and active_values and quarentine_values and upload_values and dates and times and camp_added:
            ...
            for i, fecha in enumerate(camp_added.keys()):
                try:
                    ...
                    print(f"Error al formatear fecha {fecha}: {str(e)}")
                    return Alert(
                        title="Error al formatear fechas",
                        c="red",
                        icon=DashIconify(icon="mdi:alert-circle"),
                        children=[
                            Text(f"Error al procesar la fecha {fecha}: {str(e)}")
                        ]
                    )
