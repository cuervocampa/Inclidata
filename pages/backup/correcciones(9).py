    ...
    def manejar_archivo_y_guardar(contents, guardar_clicks, cerrar_clicks, filename, tabla_json, log_cambios, corregir_tubo, nombre_archivo):
        ...
        if trigger_id == 'archivo-uploader' and contents is not None:
            ...
            try:
                ...
                return corregir_tubo_data, informacion_archivo, corregir_archivo_data, columnDefs, table_data, tabla_inicial, camp_a_graficar_data, camp_a_graficar_value, dash.no_update, dash.no_update
        ...
        if trigger_id == "guardar_tabla":
            ...
            # HAGO UN CONTROL INICIAL DE FALLOS EN LOS CAMBIOS A APLICAR. DARLE UNA VUELTA
            ...
            def camp_independiente(fecha_seleccionada, df_tabla_json):
                """
                ...
                - df_tabla_json: DataFrame con las campañas, ordenado cronológicamente descendente.
                ...
                df_tabla_json = df_tabla_json.sort_values(by='Fecha', ascending=False).reset_index(drop=True)

                # Encontrar el índice de la campaña seleccionada
                idx = df_tabla_json[df_tabla_json['Fecha'] == fecha_seleccionada].index
                idx = idx[0]
                ...
                if df_tabla_json.iloc[idx]["Referencia"]:
                ...
                if idx == 1:
                    ...
                    if df_tabla_json.iloc[0]["Referencia"]:
                        return False
                    else:
                        return True
                elif idx>1:
                    ...
                    siguiente_ref = 0
                    for _, row in df_tabla_json.iloc[idx-1::-1].iterrows():
            ...
            fecha_ref_cambio = df_tabla_json['Fecha'].min() # inicializo con la fecha más antigua
            for fila in range(len(df_tabla_json) - 1, -1, -1):
                # Obtener las filas correspondientes de cada DataFrame
                fila_json = df_tabla_json.iloc[fila]
            ...
            for fila in range(len(df_tabla_json) - 1, -1, -1):
                ...
                if acciones[fecha]['cambios']:
                    ...
                    # Cambios 2/x. Recalculo si procede
                    ...
                    corregir_tubo[fecha]['campaign_info']['active'] = fila_simulado['Activa']
                    corregir_tubo[fecha]['campaign_info']['quarentine'] = fila_simulado['Cuarentena']
                    corregir_tubo[fecha]['calc']['Reference'] = fila_simulado['Referencia']
                    corregir_tubo[fecha]['bias'] = None  # lo añado vacío
                    corregir_tubo[fecha]['spike'] = None  # lo añado vacío
            ...
            # aparto esto de momento
            if 1 ==2:
                # Recorro todo el df de la tabla comparando el original con el modificado para ver si hay cambios
                recalcular_siguientes = False # variable para que recalcule y borre la campaña
                for fila in range(len(tabla_json)-1, -1, -1):

                    # Obtener las filas correspondientes de cada DataFrame
                    fila_json = df_tabla_json.iloc[fila]
                    fila_simulado = df_simulado.iloc[fila]
                    fecha_seleccionada = fila_simulado['Fecha']
                    print('bucle', fila,'fecha seleccionada', fecha_seleccionada)
                    # la nueva fecha de referencia es:
                    if fila_simulado['Referencia'] == True:
                        # nueva referencia
                        fecha_referencia_cambio = fecha_seleccionada
                    else:
                        # referencia anterior.
                        fecha_referencia_cambio = buscar_referencia(corregir_tubo, fecha_seleccionada)

                    # Comparar las columnas de interés para ver si hay algún cambio (Condición 1/3 del bucle)
                    if (fila_json['Referencia'] != fila_simulado['Referencia'] or
                            fila_json['Activa'] != fila_simulado['Activa'] or
                            fila_json['Limpiar'] != fila_simulado['Limpiar']):
                        # compruebo si se sigue cumpliendo que recalcular_siguientes = False
                        if not camp_independiente(fecha_seleccionada, df_tabla_json):
                            recalcular_siguientes = True
                        # escribo los cambios en el json de toda la fila
                        print ('Comprobación :',fila_simulado['Referencia'])
                        corregir_tubo[fecha_seleccionada]['campaign_info']['reference'] = fila_simulado['Referencia']
                        corregir_tubo[fecha_seleccionada]['campaign_info']['active'] = fila_simulado['Activa']
                        corregir_tubo[fecha_seleccionada]['campaign_info']['quarentine'] = fila_simulado['Cuarentena']
                        corregir_tubo[fecha_seleccionada]['calc']['Reference'] = fila_simulado['Referencia']
                        corregir_tubo[fecha_seleccionada]['bias'] = None  # lo añado vacío
                        corregir_tubo[fecha_seleccionada]['spike'] = None  # lo añado vacío
                        ...
                        # se obtienen los valores calculados en función de la referencia. Ojo correcciones es todo el tubo
                        corregir_tubo = calcular_incrementos(corregir_tubo, fecha_seleccionada,
                                                            fecha_referencia_cambio)  # función externa utils/funciones_comunes.py


                    # Comparar si hay cambio sólo en Cuarentena y no se tiene que recalcular (Condición 2/3 del bucle)
                    elif (fila_json['Cuarentena'] != fila_simulado['Cuarentena'] and recalcular_siguientes):
                        # sólo se cambia el valor de Cuarentena
                        corregir_tubo[fecha_seleccionada]['campaign_info']['quarentine'] = fila_simulado['Cuarentena']

                    # Caso en que no hay cambios en esa fila pero está afectado por recalcular_siguientes (Condición 3/3 del bucle)
                    elif recalcular_siguientes:
                        # se repiten los mismos pasos que en la Condición 1/3
                        # al recalcular se deben borrar las posibles correcciones
                        corregir_tubo[fecha_seleccionada]['bias'] = None  # lo añado vacío
                        corregir_tubo[fecha_seleccionada]['spike'] = None  # lo añado vacío
                        # hay que meter en 'calc' los valores de a0-b180 calculados desde raw
                        calc_entries = []
            ...
            # Guardar también en kk.json
            ruta_kk_json = ruta_data / "kk.json"  # Ruta para guardar kk.json
            with open(ruta_kk_json, "w", encoding="utf-8") as f:
                json.dump(corregir_tubo, f, ensure_ascii=False, indent=4)

            return (dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,dash.no_update, True, "Guardados los cambios")
