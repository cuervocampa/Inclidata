    ...
    return html.Div([
        ...
        Paper(
            ...
            children=[
                ...
                dcc.Store(id='checkbox-reference-store', storage_type='memory')
            ]
        ),
...
def register_callbacks(app):
                ...
                return Paper(
                    ...
                    children=[
                        ...
                        html.Div(id='debug-output',
                                 style={'marginTop': '10px', 'padding': '10px', 'backgroundColor': '#f0f0f0'}), # borrar
    ...
    @app.callback(
        ...
        State('checkbox-reference-store', 'value')],
        prevent_initial_call=True
    )
    def execute_function_third(n_clicks, selected_value, uploaded_contents, uploaded_files, tubo, index_0, checkbox_ref_value):
        ...
        if triggered and 'import-third-button' in triggered[0]['prop_id'] and (
            ...
            try:
                ...
                print(f"🎯 CHECKPOINT: About to process references. Checkbox = {checkbox_ref_value}")
                # Manejar el caso cuando el checkbox Store está vacío (None)
                if checkbox_ref_value is None:
                    # Usar el mismo valor por defecto que se usa para el checkbox.checked
                    fechas_existentes = [key for key in tubo.keys() if
                                         key not in ['info', 'umbrales'] and isinstance(tubo.get(key), dict)]
                    checkbox_ref_value = len(fechas_existentes) == 0
                    print(f"⚠️  DEBUG: Checkbox era None, usando valor por defecto: {checkbox_ref_value}")
                    print(f"⚠️  DEBUG: Fechas existentes encontradas: {len(fechas_existentes)}")

                # Calcular las variables que dependen de la referencia y las profundidades anteriores
                # =================================================================
                # LÓGICA DE REFERENCIAS CORREGIDA
                # =================================================================

                print(f"🔍 DEBUG: checkbox_ref_value = {checkbox_ref_value} (tipo: {type(checkbox_ref_value)})")
                print(f"🔍 DEBUG: fechas_agg = {fechas_agg}")

                # PASO 1: Buscar referencia existente en el tubo
                referencia_existente = None
                for fecha_existente in tubo.keys():
                    if (fecha_existente not in ['info', 'umbrales'] and
                            isinstance(tubo.get(fecha_existente), dict) and
                            tubo[fecha_existente].get('campaign_info', {}).get('reference') is True and
                            tubo[fecha_existente].get('campaign_info', {}).get('active') is True):
                        referencia_existente = fecha_existente
                        break

                print(f"🔍 DEBUG: Referencia existente encontrada: {referencia_existente}")

                # PASO 2: Decidir qué campaña será la referencia
                if checkbox_ref_value is True:
                    # Usuario marcó checkbox: primera fecha importada será nueva referencia
                    fecha_referencia_a_usar = fechas_agg[0]
                    print(f"✅ DEBUG: Checkbox MARCADO - Nueva referencia: {fecha_referencia_a_usar}")
                elif referencia_existente:
                    # Hay referencia existente y checkbox no marcado: usar existente
                    fecha_referencia_a_usar = referencia_existente
                    print(f"🔄 DEBUG: Usando referencia existente: {fecha_referencia_a_usar}")
                else:
                    # No hay referencia existente: primera fecha será referencia por defecto
                    fecha_referencia_a_usar = fechas_agg[0]
                    print(f"⚡ DEBUG: Sin referencia - Por defecto: {fecha_referencia_a_usar}")

                # PASO 3: Marcar campañas importadas como referencia o no
                for fecha in fechas_agg:
                    if fecha == fecha_referencia_a_usar:
                        tubo[fecha]["campaign_info"]["reference"] = True
                        print(f"🟢 DEBUG: MARCANDO {fecha} como REFERENCIA")
                    else:
                        tubo[fecha]["campaign_info"]["reference"] = False
                        print(f"🔴 DEBUG: Marcando {fecha} como NO referencia")

                # PASO 4: Calcular incrementos para todas las campañas
                print(f"🧮 DEBUG: Calculando incrementos con referencia: {fecha_referencia_a_usar}")
                for fecha in fechas_agg:
                    calcular_incrementos(tubo, fecha, fecha_referencia_a_usar)

                # PASO 5: Verificación final
                print("\n" + "=" * 60)
                print("🔎 VERIFICACIÓN FINAL DE REFERENCIAS")
                print("=" * 60)
                for fecha in fechas_agg:
                    ref_status = tubo[fecha]["campaign_info"]["reference"]
                    icon = "✅" if ref_status else "❌"
                    print(f"{icon} {fecha}: reference = {ref_status}")
                print("=" * 60 + "\n")
                ...
                header = Grid([
                    Col(span=2, children=[Text("Fecha Original", fw=700, size="sm")]),
                    Col(span=1, children=[Text("Fecha TunnelData", fw=700, size="sm")]),
                    Col(span=1, children=[Text("Hora TunnelData", fw=700, size="sm")]),
                    Col(span=1, children=[Text("Activa", fw=700, size="sm")]),
                    Col(span=1, children=[Text("Cuarentena", fw=700, size="sm")]),
                    Col(span=4, children=[Text("Alarm", fw=700, size="sm")]),
                    Col(span=2, children=[Text("Subir Campaña", fw=700, size="sm")])
                ], gutter="xs", mb=10, ta="center")

                # Filas de configuración
                campaign_config_rows = []
                for i, fecha in enumerate(fechas_agg):
                    #eval_str = json.dumps(eval_por_fecha[fecha], ensure_ascii=False)
                    campaign_config_rows.append(
                        Grid([
                            Col(span=2, children=[Text(fecha, size="sm", fw=500)]),
                            Col(span=1, children=[
                                dmc.TextInput(
                            ...
                            Col(span=1, children=[
                                dmc.TimeInput(
                                    ...
                                    value=datetime.strptime(fecha.split('T')[1], "%H:%M:%S").time(),
                ...
                return (
                    Paper(
                        ...
                        children=[
                            Group([
                                DashIconify(icon="mdi:clipboard-check", width=24, c="#1976d2"),
                            ...
                            Card(
                                ...
                                children=[
                                    CardSection(
                                        ...
                                        children=[
                                            Group([
                                                DashIconify(icon="mdi:chart-line", width=20, c="#1976d2"),
                            ...
                            Card(
                                ...
                                children=[
                                    CardSection(
                                        ...
                                        children=[
                                            Group([
                                                DashIconify(icon="mdi:table-settings", width=20, c="#1976d2"),
    ...
    def update_campaign_settings(
        ...
        if n_clicks and dates and times and camp_added:
            ...
            for i, fecha in enumerate(camp_added_formateado.keys()):
                ...
                """
                try:
                    alarm_num = int(alarm_values[i])
                except (ValueError, TypeError):
                    alarm_num = 0
                camp_added_formateado[fecha]['campaign_info']['alarm'] = alarm_num
                """
    ...
    # Callback para guardar el valor del checkbox en el Store
    @app.callback(
        Output('checkbox-reference-store', 'data'),
        Input('importar-checkbox-reference', 'value'),
        prevent_initial_call=True
    )
    def save_checkbox_value(checkbox_value):
        print(f"💾 GUARDANDO valor del checkbox: {checkbox_value} (tipo: {type(checkbox_value)})")
        return checkbox_value

    #Callback de debugging temporal(REMOVER después de solucionar)

    @app.callback(
        Output('debug-output', 'children'),
        [Input('checkbox-reference-store', 'value'),
         Input('camp_added', 'data')],
        prevent_initial_call=True
    )
    def debug_reference_status(checkbox_value, camp_added):
        if not camp_added:
            return "No hay datos de campaña"

        debug_info = []
        debug_info.append(f"Checkbox value: {checkbox_value} (tipo: {type(checkbox_value)})")
        debug_info.append(f"Checkbox es True: {checkbox_value is True}")
        debug_info.append("")

        for fecha, data in camp_added.items():
            ref_status = data.get('campaign_info', {}).get('reference', 'NO DEFINIDO')
            debug_info.append(f"{fecha}: reference = {ref_status}")

        return html.Pre("\n".join(debug_info), style={'backgroundColor': '#f0f0f0', 'padding': '10px'})
