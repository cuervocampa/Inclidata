...
from utils.funciones_importar import import_RST, import_Sisgeo,import_soil_dux, insertar_camp, es_fecha_isoformat, default_value
import pprint
...
def register_callbacks(app):
        ...
        if triggered and 'import-first-button' in triggered[0]['prop_id'] and n_clicks > 0:
            if selected_files:
                ...
                return (html.Div([
                    ...
                    html.Pre(f"Info del inclinómetro en TD:\n{pprint.pformat(campaign_info)}",
                    ...
                    dbc.Row([
                        dbc.Col(dcc.Input(
                            id='index_0-input',
                            type='number',
                            value=last_index_0 if last_index_0 is not None else 1000,  # Valor predeterminado
                            style={'width': '100%'}
                        ), className='col-1'),
                        dbc.Col(html.Span(
                    ...
                    dbc.Row([
                        dbc.Col(
                            dbc.Checklist(
                                ...
                                value=[1] if campaign_info.get('latest_campaign') is None else [],
                                id="importar-checkbox-reference",
                                inline=True,  # Para mantenerlo en línea con el siguiente elemento
                                style={'transform': 'scale(1.5)', 'marginLeft': '10px', 'marginRight': '5px'}, # Duplicar el tamaño y agregar margen
                            ),
                            width="auto",  # Ajustar el ancho según el contenido
                        ),
                        dbc.Col(
                            html.Span("Es referencia"),
                            width="auto",
                        ),
                    ], ta="start", justify="start"),
    ...
    @app.callback(
        ...
        State('import-importador', 'value'),  # Estado del dropdown 'import-importador'
    ...
    def display_upload_section(n_clicks, importador_value):
        ...
        if not importador_value:
            return "Selecciona un importador para continuar"
