...
def register_callbacks(app):
    global selected_filename, temp_campaign_options, data_path
    
    @app.callback(
        [Output('import-step-2', 'children'),
         Output('selected-file-store', 'data'),
         Output('selected-file-data-store', 'data')],
    ...
    @app.callback(
        ...
        State('selected-file-data-store', 'data'),
        prevent_initial_call=True
    )
    def execute_function_third(n_clicks, selected_value, uploaded_contents, uploaded_files, selected_filename_state, selected_file_data_state):
        global temp_campaign_options

        # Definir DATA_PATH dentro del ámbito local
        # No es necesario redefinir data_path aquí
        ...
        if triggered and 'import-third-button' in triggered[0]['prop_id'] and n_clicks > 0:
            selected_filename = selected_filename_state
            selected_file_data = selected_file_data_state
    ...
    def update_campaign_settings(n_clicks, active_values, quarantine_values, upload_values):
        ...
        if n_clicks > 0:
            ...
            # Verificar si una clave es una fecha
            def is_date(key):
                try:
                    datetime.strptime(key, "%Y-%m-%d")  # Intenta convertir la clave al formato de fecha
                    return True
                except ValueError:
                    return False

            # Imprimir las claves que son fechas
            date_keys = [key for key in selected_file_data.keys() if is_date(key)]
            print('campañas totales')
            print(date_keys)
