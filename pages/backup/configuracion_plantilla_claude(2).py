                    ...
                    children=[
                        ...
                        dmc.Grid([
                            ...
                            dmc.Col([
                                ...
                                dmc.Textarea(
                                    id="graph-parameters",
                                    placeholder='"sensor":"despl_a",\n"fecha_i":"2025/06/05"',
                                    description="Introduce los parámetros en formato 'clave':'valor', uno por línea",
                                    autosize=True,
                                    minRows=4,
                                    maxRows=8,
                                ...
                                dmc.Alert(
                                    ...
                                    children=[
                                        ...
                                        html.Br(),
                                        "Ejemplo: \"sensor\":\"despl_a\", \"fecha_i\":\"2025/06/05\"",
                                    ],
                                    icon=[DashIconify(icon="mdi:information-outline")],
                                    withCloseButton=False,
                                    style={"marginBottom": "15px"}
...
def register_callbacks(app):
    ...
    @app.callback(
        [Output("graph-x", "value"),
         ...
         Output("graph-parameters", "value")],  # Añadir este output
    ...
    def fill_graph_form(selected_graph_name, store_data, current_page):
        ...
        if (not selected_graph_name or 'paginas' not in store_data or
                page_key not in store_data['paginas'] or
                'elementos' not in store_data['paginas'][page_key]):
            return 1.0, 1.0, 8.0, 6.0, "", 100, 1, 25, "Gráfico 1", ""  # Añadir "" para parámetros vacíos
        ...
        if selected_graph_name in elementos_pagina:
            ...
            if elemento["tipo"] == "grafico":
                ...
                # Convertir los parámetros a formato de texto
                parametros_texto = ""
                if parametros:
                    for clave, valor in parametros.items():
                        # Añadir comillas para valores de texto
                        if isinstance(valor, str):
                            parametros_texto += f'"{clave}":"{valor}",\n'
                        else:
                            parametros_texto += f'"{clave}":{valor},\n'
                    # Eliminar la última coma y salto de línea si hay parámetros
                    if parametros_texto:
                ...
                return (
                    ...
                    selected_graph_name,
                    parametros_texto  # Añadir el texto de parámetros
                )
    ...
    @app.callback(
        ...
        [State("graph-selector", "value"),
         ...
         State("graph-parameters", "value"),  # Añadir este state
         State("store-componentes", "data"),
         State("ep-orientation-selector", "value"),
         State("ep-page-number", "value")],
        prevent_initial_call=True
    )
    def crear_actualizar_grafico(n_clicks, selected_name, x, y, ancho, alto, script,
        ...
        if parametros_texto and parametros_texto.strip():
            try:
                # Preparar el texto para convertirlo a JSON
                ...
                # Si hay un error en el formato, mostrar un mensaje pero continuar
                print(f"Error al procesar parámetros: {str(e)} {parametros_texto}")
        ...
        grafico_datos = {
            ...
            "configuracion": {
                "script": script,
                "parametros": parametros  # Incluir los parámetros procesados
            },
            "estilo": {
                "opacidad": opacidad,
                "reduccion": reduccion
