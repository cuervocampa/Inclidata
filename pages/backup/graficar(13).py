...
from utils.funciones_graficar import obtener_fecha_desde_slider, obtener_color_para_fecha, extraer_datos_fecha, add_traza, interpolar_def_tubo, load_module_dynamically, cargar_valores_actuales
...
def register_callbacks(app):
    ...
    def cargar_plantilla_seleccionada(nombre_plantilla, data, eje, orden, color_scheme,
                                      escala_desplazamiento, escala_incremento,
                                      valor_positivo_desplazamiento, valor_negativo_desplazamiento,
                                      valor_positivo_incremento, valor_negativo_incremento,
                                      fecha_inicial, fecha_final, total_camp, ultimas_camp, cadencia_dias):
        """
        Carga el JSON de la plantilla seleccionada y genera los campos editables
        para textos y gráficos mostrando sus rutas completas en el JSON.
        """
        ...
        try:
            ...
            if not os.path.exists(ruta_json):
                return None, [
                    dmc.Alert(f"No se encontró el archivo JSON para la plantilla '{nombre_plantilla}'",
                              c="red")]
            ...
            # Contenedor para todos los campos editables
            campos_editables = []

            # SECCIÓN DE TEXTOS EDITABLES
            campos_editables.append(
                dmc.Divider(
                    label="TEXTOS EDITABLES",
                    labeljustify="center",
                    size="md",
            ...
            # Contador para textos encontrados
            textos_encontrados = 0

            # Buscar todos los elementos de texto editables en todas las páginas
            for num_pagina, pagina in data_json.get("paginas", {}).items():
                ...
                for nombre_elemento, elemento in elementos.items():
                    # Verificar si es un elemento de texto editable
                    if (elemento.get("tipo") == "texto" and
                            "contenido" in elemento and
                            elemento["contenido"].get("editable", True)):
                        textos_encontrados += 1
                        # Texto actual del elemento
                        texto_actual = elemento.get("contenido", {}).get("texto", "")

                        # Crear un campo para editar este texto con ruta completa
                        campos_editables.append(
                            dmc.Group([
                                # Etiqueta de ruta completa en el JSON
                                dmc.Text(
                                    f"[paginas][{num_pagina}][elementos][{nombre_elemento}][contenido][texto]",
                                    fw="bold",
                                    style={"width": "50%"}
                                ),
            ...
            # Mensaje si no hay textos editables
            if textos_encontrados == 0:
                campos_editables.append(
                    dmc.Alert("No se encontraron campos de texto editables en esta plantilla",
                              c="yellow")
                )
            ...
            # Verificar scripts disponibles en biblioteca_graficos (MODIFICADO)
            directorio_graficos = "biblioteca_graficos"
            scripts_disponibles = []

            # Comprobar scripts en estructura de carpetas
            if os.path.exists(directorio_graficos):
                for item in os.listdir(directorio_graficos):
                    ...
                    # Solo buscar en subcarpetas, no en archivos
                    if os.path.isdir(path_item):
                        # Verificar si dentro de la subcarpeta existe un archivo PYTHON con el mismo nombre
                        py_path = os.path.join(path_item, f"{item}.py")
                        if os.path.exists(py_path):
                            # Añadir a la lista de scripts disponibles
                            scripts_disponibles.append({"label": item, "value": item})
                            print(f"Script encontrado: {item}")

            # Si no hay scripts disponibles, mostrar un mensaje informativo
            if not scripts_disponibles:
                print("No se encontraron scripts válidos en la carpeta biblioteca_graficos")
                # Podemos añadir un valor predeterminado para no dejar vacío el selector
                scripts_disponibles = [{"label": "No hay scripts disponibles", "value": ""}]

            # Contador para gráficos encontrados
            graficos_encontrados = 0

            # Buscar todos los elementos de tipo gráfico en todas las páginas
            for num_pagina, pagina in data_json.get("paginas", {}).items():
                elementos = pagina.get("elementos", {})

                for nombre_elemento, elemento in elementos.items():
                    # Verificar si es un elemento de tipo gráfico
                    if elemento.get("tipo") == "grafico":
                        graficos_encontrados += 1

                        # Crear una tarjeta para cada gráfico
                        tarjeta_grafico = []

                        # Título del gráfico con el nombre del elemento real
                        tarjeta_grafico.append(
                            dmc.CardSection(
                                dmc.Text(f"{nombre_elemento}", fw="bold", size="lg"),
                                withBorder=True,
                                inheritPadding=True,
                        ...
                        # Verificar si el script está en la lista de opciones disponibles (MODIFICADO)
                        script_en_opciones = any(d['value'] == script_valor_sin_extension for d in scripts_disponibles)

                        # Establecer el valor por defecto del selector (MODIFICADO)
                        ...
                        tarjeta_grafico.append(
                            dmc.Group([
                                dmc.Text(
                                    f"[paginas][{num_pagina}][elementos][{nombre_elemento}][configuracion][script]",
                                    style={"width": "30%"}
                                ),
                                html.Div([
                                    dmc.Select(
                                        id={"type": "script-grafico", "pagina": num_pagina,
                                            "elemento": nombre_elemento},
                                        value=valor_selector,  # Será None si el script no está en opciones
                                        data=scripts_disponibles,
                                        placeholder="Seleccione un script",
                                        style={"width": "100%", "marginBottom": "5px"}
                                    ),
                                    dmc.Text(script_status,
                                             c=status_color,
                                             size="sm")
                                ], style={"width": "70%"})
                            ], style={"padding": "10px"})
                        ...
                        # Campos para cada parámetro con ruta completa
                        parametros = configuracion.get("parametros", {})
                        if parametros:
                            tarjeta_grafico.append(
                                dmc.Text("Parámetros:", fw="bold", style={"padding": "10px 10px 0 10px"})
                            )

                            for param_nombre, param_valor in parametros.items():
                                # AQUÍ ES DONDE APLICAMOS LA SUSTITUCIÓN DE $CURRENT
                                valor_display = param_valor
                                if isinstance(param_valor,
                                              str) and param_valor == "$CURRENT" and param_nombre in current_values:
                                    valor_display = current_values[param_nombre]
                                    # Opcional: Convierte el valor a string para mostrarlo
                                    valor_display = str(valor_display)
                                    # Estilo especial para campos que vienen de $CURRENT
                                    estilo_campo = {
                                        "width": "50%",
                                        "backgroundColor": "#e3f2fd",  # Fondo azul claro
                                        "border": "1px dashed #90caf9"  # Borde punteado azul
                                    }
                                else:
                                    # Estilo normal para campos con valores directos
                                ...
                                tarjeta_grafico.append(
                                    dmc.Group([
                                        dmc.Text(
                                            f"[paginas][{num_pagina}][elementos][{nombre_elemento}][configuracion][parametros][{param_nombre}]",
                                            style={"width": "50%"}
                                        ),
                                        dmc.TextInput(
                                            id={"type": "param-grafico", "pagina": num_pagina,
                                                "elemento": nombre_elemento, "param": param_nombre},
                                            value=valor_display,  # Usamos el valor procesado
                                            style=estilo_campo
                                        )
                                    ], style={"padding": "0px 10px 10px 10px"})
                                )

                        # Si no hay parámetros, mostrar un mensaje
                        else:
                            tarjeta_grafico.append(
                                dmc.Stack([
                                    dmc.Text("No hay parámetros configurados",
                                             c="dimmed",
                                             style={"padding": "0px 10px 10px 10px"}),
                                    dmc.Button(
                                        "Añadir parámetros por defecto",
                                        id={"type": "btn-add-params", "pagina": num_pagina,
                                            "elemento": nombre_elemento},
                                        variant="outline",
                                        size="sm",
                                        style={"width": "fit-content", "margin": "0px 10px 10px 10px"}
                                    )
                                ])
                            )

                        # Agregar la tarjeta completa a los campos editables
                        campos_editables.append(
                            dmc.Card(
                                children=tarjeta_grafico,
                                withBorder=True,
                                shadow="sm",
                                radius="md",
                                style={"marginBottom": "20px"}
                ...
                if graficos_encontrados == 0:
                    campos_editables.append(
                        dmc.Alert("No se encontraron gráficos configurables en esta plantilla",
                                  c="yellow")
                    )
            # Agregar un tooltip explicativo para los valores $CURRENT
            if graficos_encontrados > 0:
                campos_editables.append(
                    dmc.Alert(
