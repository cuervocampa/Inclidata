...
def register_callbacks(app):
    ...
    def process_json_upload(contents, filename, store_data):
        if contents is None:
            return dash.no_update, "", "blue", True, dash.no_update, dash.no_update
        ...
        try:
            ...
            orientation = "portrait"
            if "1" in new_store_data['paginas'] and "configuracion" in new_store_data['paginas']["1"]:
                orientation = new_store_data['paginas']["1"]["configuracion"].get("orientacion", "portrait")

            return new_store_data, f"JSON '{filename}' cargado correctamente", "green", False, "1", orientation

        except Exception as e:
            return dash.no_update, f"Error al cargar JSON: {str(e)}", "red", False, dash.no_update, dash.no_update
