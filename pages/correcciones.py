...
from pages.correcciones_layout import layout
...
def register_callbacks(app):
    ...
    def manejar_archivo_y_guardar(contents, guardar_clicks, cerrar_clicks, filename, tabla_json, log_cambios, corregir_tubo, nombre_archivo):
        ...
        if trigger_id == 'archivo-uploader' and contents is not None:
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
                return dash.no_update, f"Error al procesar el archivo: {str(e)}", dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    ...
    def cambio_json_spikes(data, cellValueChanged, camp_graficar, table_data):#, camp_corregida):
        ...
        for entrada in correccion[camp_graficar]["calc"]:
            entrada["dev_a"] = round((entrada["a0"] - entrada["a180"]) / 2, 2) if entrada.get("a0") is not None and entrada.get("a180") is not None else None
            entrada["dev_b"] = round((entrada["b0"] - entrada["b180"]) / 2, 2) if entrada.get("b0") is not None and entrada.get("b180") is not None else None
            entrada["checksum_a"] = round(entrada["a0"] + entrada["a180"], 4) if entrada.get("a0") is not None and entrada.get("a180") is not None else None
            entrada["checksum_b"] = round(entrada["b0"] + entrada["b180"], 4) if entrada.get("b0") is not None and entrada.get("b180") is not None else None
    ...
    def actualizar_spikes_table(data, fecha_seleccionada, fechas, n_spikes, criterio):
        ...
        for profundidad in dfs['a0'].index.tolist():
            ...
            for variable in ['a0', 'a180', 'b0', 'b180']:
                val = dfs[variable].loc[profundidad, fecha_seleccionada]
                row[variable] = None if pd.isna(val) else val
            for columna_c, valores_c in columnas_c.items():
                val = valores_c.loc[profundidad]
                row[columna_c] = None if pd.isna(val) else val
            rowData.append(row)
      ...
      try:
        ...
        if corr_spikes: # esto es heredado, no sé si realmente es necesario
            ...
            if (df_bias_table['Prof_inf'].sum() + df_bias_table['Prof_sup'].sum()) == 0 or sugerir or triggered_input == 'empotramiento': # condición repetida?? si no hay correc o sugerir
                ...
                # Convierte el DataFrame actualizado en una lista de diccionarios, usando None para los NaNs
                rowData_actualizado = df_bias_table.where(pd.notnull(df_bias_table), None).to_dict(orient='records')

                import time
                return rowData_actualizado, time.time()
    ...
    def cambios_json_bias(cellValueChanged, change_flag, corr_spikes, camp_a_graficar, bias_table, corregir_tubo ):
      try:
        ...
        dict_df_bias = df_bias_corr.where(pd.notnull(df_bias_corr), None).to_dict(orient='records')
        ...
        if trigger_id == "save_json":
                    ...
                    'calc': [
                        {
                            ...
                            'depth': item['depth'],
                            'a0': round(item['a0'], 2) if item.get('a0') is not None else None,
                            'a180': round(item['a180'], 2) if item.get('a180') is not None else None,
                            'b0': round(item['b0'], 2) if item.get('b0') is not None else None,
                            'b180': round(item['b180'], 2) if item.get('b180') is not None else None,
                            'checksum_a': round(item['a0'] + item['a180'], 4) if item.get('a0') is not None and item.get('a180') is not None else None,
                            'checksum_b': round(item['b0'] + item['b180'], 4) if item.get('b0') is not None and item.get('b180') is not None else None,
                            'dev_a': round((item['a0'] - item['a180']) / 2, 2) if item.get('a0') is not None and item.get('a180') is not None else None,
                            'dev_b': round((item['b0'] - item['b180']) / 2, 2) if item.get('b0') is not None and item.get('b180') is not None else None
                        }
            ...
            with open(ruta_json, "r+", encoding="utf-8") as f:
                ...
                if fecha_seleccionada in data:
                    ...
                    print(f"La fecha {fecha_seleccionada} no existe en el archivo JSON.")
                ...
                print(f"Editando el archivo: {ruta_json}")
