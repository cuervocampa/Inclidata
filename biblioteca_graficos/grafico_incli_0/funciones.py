...
def extraer_datos_fecha(fecha, data, eje):
    ...
    try:
        ...
        incr_checksum_a = [punto.get("incr_checksum_a", 0) for punto in calc_data]
        incr_checksum_b = [punto.get("incr_checksum_b", 0) for punto in calc_data]
        ...
        return {
            ...
            'incr_checksum_a': incr_checksum_a,
            'incr_checksum_b': incr_checksum_b
        }
...
def obtener_datos_sensor(datos, sensor):
    """
    Extrae los datos del sensor especificado y devuelve el título correspondiente.

    Args:
        datos (dict): Diccionario con los datos extraídos de la fecha.
        sensor (str): Tipo de sensor.

    Returns:
        tuple: (x_data, titulo)
    """
    sensor_map = {
        "desp_a": (datos['desp_a'], "Desplazamiento A"),
        "desp_b": (datos['desp_b'], "Desplazamiento B"),
        "incr_dev_abs_a": (datos['incr_dev_abs_a'], "Incremental A"),
        "incr_dev_abs_b": (datos['incr_dev_abs_b'], "Incremental B"),
        "checksum_a": (datos['checksum_a'], "Checksum A"),
        "checksum_b": (datos['checksum_b'], "Checksum B"),
        "desp_total": (datos['desp_total'], "Desplazamiento Total"),
        "incr_checksum_a": (datos['incr_checksum_a'], "Incr Checksum A"),
        "incr_checksum_b": (datos['incr_checksum_b'], "Incr Checksum B"),
    }

    return sensor_map.get(sensor, (datos['desp_a'], "Desplazamiento A"))