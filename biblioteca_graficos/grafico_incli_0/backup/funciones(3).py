...
def obtener_leyenda_umbrales(data):
    ...
    if not data or 'umbrales' not in data or 'deformadas' not in data['umbrales']:
    ...
    try:
        ...
        print(f"Error al procesar umbrales: {e}")
...
def configurar_ejes(ax, sensor, escala_desplazamiento, escala_incremento,
    ...
    try:
        ...
        if etiqueta_eje_x:
            ax.set_xlabel(etiqueta_eje_x, fontfamily='Arial')
        ...
        ax.set_ylabel(titulo_eje_y, fontfamily='Arial')
        ...
        # Configurar fuente Arial para los ticks
