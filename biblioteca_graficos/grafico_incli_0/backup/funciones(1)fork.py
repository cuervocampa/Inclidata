...
def agregar_umbrales(ax, data, leyenda_umbrales, eje, sensor, fecha_slider, incluir_todos=False):
    ...
    if not data or 'umbrales' not in data or 'valores' not in data['umbrales']:
        return
    ...
    for deformada in deformadas:
        ...
        color = color_espanol  # Aquí iría la conversión si es necesario

        # Establecer parámetros de línea
        grosor = 2
        ...
        try:
            ...
            # Asegurarse de usar líneas discontinuas para los umbrales
            ax.plot(eje_X, eje_Y, color=color, linestyle='--', linewidth=grosor, label=f"{deformada}")
        except Exception as e:
            print(f"Error al procesar umbral {deformada}: {str(e)}")

    # Añadir umbrales solo para la leyenda si es necesario
    if incluir_todos:
        for deformada, color_espanol in leyenda_umbrales.items():
            if deformada not in deformadas_mostradas:
                # Para la leyenda, solo añadimos líneas invisibles
                ax.plot([], [], color=color_espanol, linestyle='--', linewidth=2, label=deformada)
...
def configurar_ejes(ax, sensor, escala_desplazamiento, escala_incremento,
    ...
    # Añadir etiqueta al eje X si se proporciona
    if etiqueta_eje_x:
    ...
    # Ajustar el gráfico para dejar espacio para la leyenda de umbrales
    plt.tight_layout()

    # Asegurarse de que hay espacio en la parte inferior para la leyenda de umbrales
    bottom_margin = 0.15  # Proporciona espacio en la parte inferior
