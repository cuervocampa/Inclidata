...
def grafico_incli_series_0(data, parametros):
    ...
    Returns:
        str: Imagen de la leyenda en formato PNG codificada en base64 (data URL).
    """
    ...
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # Eliminar todos los márgenes

    # Ocultar los ejes ya que solo queremos la leyenda
    ax.set_axis_off()
    ...
    # PASO 8: Crear la leyenda
    legend = ax.legend(
        handles=handles,
        loc='upper left',  # Posición en esquina superior izquierda
        bbox_to_anchor=(0, 1),  # Anclar en esquina superior izquierda
        frameon=True,
        framealpha=0.9,
        edgecolor='lightgray',
        fontsize=8,  # Tamaño de letra fijo a 8 puntos
        title="Fechas de campaña",
        alignment='left'  # Justificación del texto a la izquierda
    )

    # Ajustar el contenedor de la leyenda manualmente para que use todo el ancho
    legend._legend_box.align = "left"
    legend.set_draggable(False)

    # Ajustar la leyenda para usar más espacio horizontal
    plt.tight_layout(pad=0)
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0.1)  # Pequeño margen inferior

    # PASO 9: Guardar como imagen en memoria
    buffer = io.BytesIO()
    plt.savefig(buffer, format='png', dpi=dpi, bbox_inches='tight')  # Volver a usar bbox_inches='tight'
